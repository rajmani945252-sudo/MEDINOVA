...
const checkSymptoms = async (req, res) => {
  ...
  const suggestions = found.size > 0 ? [...found] : ['General Physician'];
  ...
  try {
    ...
    const [doctors] = await db.promise().query(`
      ...
      WHERE u.role = 'doctor'
      AND COALESCE(d.specialization, 'General Physician') IN (${placeholders})
    `, suggestions);

    return res.status(200).json({ suggestions, doctors });
  } catch (err) {
...
const getHealthTips = (req, res) => {
  const tips = [
    { icon: 'water', title: 'Stay Hydrated', tip: 'Drink at least 8 glasses of water daily to keep your body functioning properly.' },
    { icon: 'run', title: 'Exercise Daily', tip: '30 minutes of moderate exercise daily reduces risk of heart disease by 35%.' },
    { icon: 'sleep', title: 'Sleep Well', tip: 'Adults need 7-9 hours of sleep. Poor sleep increases risk of obesity and diabetes.' },
    { icon: 'diet', title: 'Eat Balanced Meals', tip: 'Include fruits, vegetables, whole grains, and lean proteins in every meal.' },
    { icon: 'calm', title: 'Manage Stress', tip: 'Chronic stress affects heart health. Practice meditation or deep breathing daily.' },
    { icon: 'stop', title: 'Quit Smoking', tip: 'Smoking causes 1 in 5 deaths annually. Quitting adds years to your life.' },
    { icon: 'brush', title: 'Oral Hygiene', tip: 'Brush twice daily and floss once. Poor oral health is linked to heart disease.' },
    { icon: 'sun', title: 'Get Sunlight', tip: '15-20 minutes of sunlight daily boosts Vitamin D and improves mood.' },
    { icon: 'checkup', title: 'Regular Checkups', tip: 'Annual health checkups catch problems early when they are easiest to treat.' },
    { icon: 'wash', title: 'Wash Your Hands', tip: 'Proper handwashing for 20 seconds prevents 80% of common infections.' },
  ];

  const today = new Date().getDay();
  const dailyTips = [tips[today], tips[(today + 1) % tips.length], tips[(today + 2) % tips.length]];
  return res.status(200).json(dailyTips);
};
...
const saveHealthRecord = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Health record saved!' });
  } catch (err) {
    return res.status(500).json({ message: 'Server error', error: err.message });
  }
};
